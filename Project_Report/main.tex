...
The tokens we picked is Tierion, Aragon, and Bitqy. Because we cannot find token126, hms and lino in token pirce folder, so we moved down and picked Bitqy as our third token.
...
bitqyFiltered <-filter(bitqy,tokenAmount < decimals * supply)
